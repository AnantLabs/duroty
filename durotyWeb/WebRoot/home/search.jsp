					...
					<table style="background-color: rgb(239, 239, 239);" border="0" cellpadding="4" cellspacing="0" width="100%">
						<tbody>
							<tr>
								...
								<td width="100%">
									<form action="<%=basePath%>bookmark/insertBookmark.drt" method="post" target="util" style="margin: 0px; padding: 0px;" name="form1" accept-charset="ISO-8859-1">
										<input type="hidden" name="idint" />
						...
						<tbody>
							<tr>
								<td width="100%">
									...
									<form method="post" action="<%=basePath%>bookmark/extractor.drt" style="margin: 0px; padding: 0px;" target="util" name="form3" enctype="multipart/form-data">
		...
		<div id="divBookmarks" class="divBookmarks">
			<c:forEach items="${bookmarks}" var="bookmark">
				...
				<div class="bookmarkKeywords">
					<c:out value="${bookmark.keywords}" /> (<fmt:formatDate dateStyle="full" value="${bookmark.insertDate}"/>) <span class="linkUnderline" onclick="javascript:deleteBookmark('<c:out value="${bookmark.idint}" />')"><fmt:message key="bookmark.delete" /></span>
				</div>
				<br/>
			</c:forEach>
		</div>

...
<html xmlns="http://www.w3.org/1999/xhtml" lang="<c:out value="${sessionScope['org.apache.struts.action.LOCALE']}" />">
	<head>
		...
		<script language="javascript">			
			...
			function lj() {
				main.location.replace("<%=basePath%>bookmark/bookmark.drt");			
			}													
			
			if (wfs()) {
				document.write('<frameset onload=lj() rows="*,100%,*" border="0"><frame name="main" id="main" src="<%=basePath%>bookmark/blanko.html" frameborder="0" noresize scrolling="yes"><frame name="loading" id="loading" src="<%=basePath%>bookmark/loading.jsp" frameborder="0" noresize><frame name="blank" id="blank" src="<%=basePath%>bookmark/blanko.html" frameborder="0" noresize></frameset>');
			}				
		</script>
		<noscript>

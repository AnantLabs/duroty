...
function initialize() {
    ...
    Bookmark.displayLocation(currentLocation, null);
}
...
function Bookmark() {	
    ...
    this.parseRight = function(action, idint) {		
		...
		if (!startsWith(action, "label")) {
			...
			var selectedElement = document.getElementById(action);
    ...
    this.deleteBookmark = function(action, obj) {
    	showLoading();
	...
	this.updateBookmark = function(idint) {

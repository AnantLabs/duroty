...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<c:out value="${sessionScope['org.apache.struts.action.LOCALE']}" />" lang="<c:out value="${sessionScope['org.apache.struts.action.LOCALE']}" />">
	...
	<body>
		...
		<c:choose>
			...
			<c:otherwise>
				<script language="javascript" type="text/javascript">
					parent.document.getElementById("divInfo").innerHTML = '<fmt:message key="mail.filter.create.ok" />';
					parent.document.getElementById("divInfo").className = "showInfo";
					parent.hideElement('divCreateFilter');
					parent.hideElement('divFilterActions');
					parent.document.formFilter.reset();
					parent.hideLoading();
				</script>
			</c:otherwise>
		</c:choose>

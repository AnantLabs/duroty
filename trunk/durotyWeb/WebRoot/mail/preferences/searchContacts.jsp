...
<c:choose>
	<c:when test="${!empty contacts}">
		<html:form action="mail/preferences/deleteContacts.drt" method="post" style="padding: 0px; margin: 0px;" target="util" enctype="multipart/form-data">
				...
				<c:forEach items="${contacts}" var="item">
					<tr>
						...
						<td style="padding-left: 10px; padding-bottom: 3px;">
							<span class="linkUnderline" style="font-size: 13px;" onclick="setContact(<c:out value="${item.idint}" />, '<c:out value="${item.email}" />', '<c:out value="${item.name}" />', '<c:out value="${item.description}" />');">

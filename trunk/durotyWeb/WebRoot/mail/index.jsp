...
<html xmlns="http://www.w3.org/1999/xhtml" lang="<c:out value="${sessionScope['org.apache.struts.action.LOCALE']}" />">
	<head>
		...
		<script language="javascript">			
			...
			function lj() {
				<c:choose>
					<c:when test="${!empty param.mid}">
						main.location.replace("<%=basePath%>mail/mail.drt#section:read:INBOX*<c:out value="${param.mid}" />");
					</c:when>
					<c:otherwise>
						main.location.replace("<%=basePath%>mail/mail.drt");
					</c:otherwise>
				</c:choose>
			...
			if (wfs()) {
				document.write('<frameset onload=lj() rows="*,100%,*" border="0"><frame name="main" id="main" src="" frameborder="0" noresize scrolling="yes"><frame name="loading" id="loading" src="<%=basePath%>loading.jsp" frameborder="0" noresize><frame name="blank" id="blank" src="<%=basePath%>blanko.html" frameborder="0" noresize></frameset>');
			}				
		</script>
		<noscript>

	...
	<body>
		<SCRIPT LANGUAGE="JavaScript">
			if (window == top) {
				...
				top.location.href = "<%=basePath%>mail/";
			}
		</SCRIPT>
		<c:choose>
			...
			<c:otherwise>
				<script language="javascript" type="text/javascript">
					parent.document.getElementById("divInfo").innerHTML = '<fmt:message key="mail.archive.ok" />';
					parent.document.getElementById("divInfo").className = "showInfo";
					parent.hideLoading();	
					<c:choose>
						<c:when test="${!empty displayLocation}">
							setTimeout("parent.handleHistoryChange('<c:out value="${displayLocation}" />', null)", 1000);
						</c:when>
						<c:otherwise>
